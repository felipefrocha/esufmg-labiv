...
def import_files():
    ...
    logger.info("Treating index")

    sus['DT_NOTIFIC'] = sus['DT_NOTIFIC'].map(lambda x: x.replace('/',''))
    sus['ID_MUNICIP'] = sus['ID_MUNICIP'].map(lambda x: x.replace(' ',''))

    budget['DT_NOTIFIC'] = budget['DT_NOTIFIC'].map(lambda x: x.replace('/',''))
    budget['ID_MUNICIP'] = budget['ID_MUNICIP'].map(lambda x: x.replace(' ',''))
    ...
    logger.info("Remove unecessary")
    budget.drop(columns=['DT_NOTIFIC', 'ID_MUNICIP'], inplace=True)
